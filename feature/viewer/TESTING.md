...
QA-инженеры могут проверить CPU-путь двумя способами:

1. **Через UI**: откройте экран настроек (секретное dev-меню) и в секции «Улучшение» включите переключатель **«Принудительно CPU backend»**. Изменение сразу вызывает `NativeEnhanceController.setForceCpuOverride(true)` и сохраняется в DataStore, поэтому после перезапуска настройка останется.
2. **Через переменную окружения**: перед запуском процесса установите `ENHANCE_FORCE_CPU=1`. Это можно сделать, добавив переменную в Run/Debug configuration Android Studio или установив временный wrapper: `adb shell setprop wrap.com.kotopogoda.uploader "logwrapper env ENHANCE_FORCE_CPU=1"`, затем перезапустите приложение. После тестов очистите wrapper (`adb shell setprop wrap.com.kotopogoda.uploader ''`).

В телеметрии (`native_controller_init`, `native_delegate_status`) такие сессии будут помечены `forceCpu_reason="user_override"`, что упрощает анализ логов.
