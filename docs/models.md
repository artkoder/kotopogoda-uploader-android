...
- Этот файл читает Gradle-задача `fetchModels` и проверяет контрольные суммы во время сборки приложения. Отключённые модели (например, `restormer_fp32`) пропускаются, поэтому APK продолжает включать только актуальные FP16 веса.
