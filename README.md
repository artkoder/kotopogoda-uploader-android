...
После подтверждённой постановки задачи и удачного ответа от сервера фоновый `PollStatusWorker` удаляет как загруженный файл (включая временный улучшенный результат), так и исходный кадр из выбранной папки. Логи `cleanup_ok`/`cleanup_error` в категории `UPLOAD/POLL` содержат поле `cleanup_strategy=post_upload`, статус обоих удалений (`upload_state`, `source_state`) и признак совпадения URI (`source_same_as_upload`).

`ViewerViewModel` фиксирует стратегию очистки в момент клика по публикации (`publish_click` теперь включает `source_same_as_upload`) и следит за завершением фоновой загрузки. После подтверждённой отправки выполняется повторная очистка: исходный документ удаляется через `ContentResolver`/`DocumentFile`, а временный результат enhancement — напрямую с диска. Итоги фиксируются в `cleanup_ok`/`cleanup_error` и `enhance_cleanup`, где дополнительно сохраняются количество попыток удаления и признак совпадения URI.
